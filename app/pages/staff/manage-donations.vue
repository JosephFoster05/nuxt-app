...
const setProcessing = (id, v) => {
  processing.value = { ...processing.value, [id]: v };
};
...
<template>
        ...
        <tr v-for="d in filteredDonations" :key="d.donation_id">
          ...
          <td>
            <img
              v-if="d.image_url"
              :src="d.image_url"
              :alt="d.clothing_name || 'donation image'"
          ...
          <td>
            <button @click="approve(d)" :disabled="processing[d.donation_id]">
              Approve
            </button>
            <button
              @click="deny(d)"
              :disabled="processing[d.donation_id]"
  ...
  <section>
    ...
    <div
      v-else-if="
        filteredDonationsDenied && filteredDonationsDenied.length === 0
      "
    >
      No denied donations.
    </div>
  ...
  <section>
    ...
    <div
      v-else-if="
        filteredDonationsAccepted && filteredDonationsAccepted.length === 0
      "
    >
    ...
    <table v-else class="donations-table" border="1" cellpadding="6">
      ...
      <tbody>
        <tr v-for="d in filteredDonationsAccepted" :key="d.donation_id">
          ...
          <td>{{ d.donation_gender ?? "-" }}</td>
          <td>
            <button @click="deny(d)" :disabled="processing[d.donation_id]">
              Deny
            </button>
          </td>
        </tr>
